...
---
# Overview
This project aims to determine whether a particular medical regimen improves health, as measured by weight loss and self-reported health (SRH).  All starting data were already anonymized.  
...
ggplot(DF, aes(x=trt, y=weightGain)) +
  geom_jitter(size=0.5) + xlab("Treatment") + ylab("Weight Gain")
```
...
####
The motivation is that I'd like to see if I can sub-divide the women in some way as to see if those who gain weight have some predictive pattern.  
####
...
### Separately Modeling Men and Women
...
```
Again, consistent with the *modelWomenMVR* above, age is not an explanatory variable, with the possible exception of women under 25 and women over 55, had we broken *age* into 3 distinct ranges.  
